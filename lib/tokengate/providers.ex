defmodule Tokengate.Providers do
  ...
  # ---------------------------------------------------------------------------
  # Model catalog (models.dev mirror)
  # ---------------------------------------------------------------------------

  # The picker row carries only what the modal renders, and the description is
  # cut in SQL: the mirror holds ~3000 models and a LiveView keeps the whole list
  # in memory to filter on every keystroke (same shape as the provider picker).

  @doc """
  Every model models.dev publishes (or a provider lists), with how many
  providers serve it — the source list for the model picker.

  Filtering happens in memory on purpose, like the provider mirror: the search
  must feel instant on every keystroke. `:provider_count` is what turns a row
  into "routable: N proveedores" and it is recomputed here because it lives in
  ...
  def catalog_picker_models do
    ...
    from(m in CatalogModel,
      ...
      select: %{
        ...
        description: fragment("left(?, 200)", m.description)
      }
    )
    |> Repo.all()
  ...
  def count_catalog_models(status) when status in ~w(active stale) do
    Repo.aggregate(from(m in CatalogModel, where: m.status == ^status), :count)
  end

  @doc "Number of provider × model offers in the mirror."
  def count_catalog_offers do
    Repo.aggregate(CatalogModelOffer, :count)
  end

  @doc "A catalog model by its models.dev id (nil when unknown)."
  def get_catalog_model(key) when is_binary(key), do: Repo.get(CatalogModel, key)
  def get_catalog_model(_), do: nil

  @doc "The catalog model ids an operator's `models` row was already created from."
  def registered_catalog_model_keys do
    from(m in Model,
      where: not is_nil(m.catalog_model_key),
      select: m.catalog_model_key,
      distinct: true
    )
    |> Repo.all()
    |> MapSet.new()
  ...
  @doc """
  The providers that serve a catalog model, cheapest first.

  Each entry is `%{offer: %CatalogModelOffer{}, provider: %Provider{}}` and only
  providers that exist in `providers` are returned: an offer whose provider was
  never materialized (or was deleted) cannot be routed to, so offering it in the
  modal would be a dead end. Credentials are preloaded for the "ya tiene N API
  keys" hint.
  """
  def providers_serving(model_key) when is_binary(model_key) do
    offers =
      from(o in CatalogModelOffer,
        where: o.model_key == ^model_key and o.status == "active",
        order_by: [asc_nulls_last: o.cost_input, asc: o.provider_key]
      )
      |> Repo.all()

    keys = Enum.map(offers, & &1.provider_key)

    providers =
      from(p in Provider, where: p.key in ^keys, preload: [:credentials])
      |> Repo.all()
      |> Map.new(&{&1.key, &1})

    offers
    |> Enum.map(fn offer -> %{offer: offer, provider: Map.get(providers, offer.provider_key)} end)
    |> Enum.filter(& &1.provider)
  end

  def providers_serving(_), do: []

  @doc """
  The offer of ONE provider for a catalog model (nil when that provider does not
  serve it) — used to prefill `provider_model` and the manual prices when the
  operator picks a provider in the modal.
  """
  def offer_for(model_key, provider_key)

  def offer_for(model_key, provider_key) when is_binary(model_key) and is_binary(provider_key) do
    Repo.one(
      from(o in CatalogModelOffer,
        where: o.model_key == ^model_key and o.provider_key == ^provider_key
      )
    )
  end

  def offer_for(_model_key, _provider_key), do: nil
