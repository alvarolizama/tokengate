defmodule Tokengate.Providers.CatalogSeed do
  ...
  Both halves are idempotent, and neither ever overwrites what a refresh
  brought:
  ...
  The lab counters on `catalog_sync_state` (`labs_inserted`, …) mean "written
  by the catalog sync that just ran", which on a fresh boot is this seed; the
  refresh owns the same counters from then on.
  """

  require Logger
  import Ecto.Query, only: [from: 2]

  alias Tokengate.Providers.{Catalog, CatalogProvider, CatalogSyncState, Lab, LabCatalog}
