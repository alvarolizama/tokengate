defmodule Tokengate.Routing.StickyTracker do
  ...
  mapping `{api_key_hash, model_alias_id}` to `model_provider_id`.
  ...
  def get(api_key_hash, model_alias_id) do
    try do
      ...
      [{_, model_provider_id}] -> model_provider_id
      [] -> nil
    end
  end
  ...
  def handle_cast({:put, api_key_hash, model_alias_id, model_provider_id}, state) do
    :ets.insert(@table, {{api_key_hash, model_alias_id}, model_provider_id})
    {:noreply, state}
  end
  ...
  def handle_call({:clear_all_for_provider, model_provider_ids}, _from, state) do
    ...
    :ets.foldl(
      fn
        {{api_key_hash, model_alias_id} = key, model_provider_id}, acc
        when is_binary(api_key_hash) and is_binary(model_alias_id) ->
          if MapSet.member?(id_set, model_provider_id) do
