defmodule Tokengate.Routing.Strategy do
  ...
  A strategy receives the list of candidate `Tokengate.Providers.AliasProvider`
  ...
  injects availability through the `:available?` predicate in `opts`:
    ...
    * `:available?` – `fn(alias_provider) -> boolean`. Defaults to
      `fn _ -> true end`. The circuit breaker is injected here by the caller.

  ## Return value

    * `{:ok, %Tokengate.Providers.AliasProvider{}}` – the selected provider.
    * `{:error, :no_available_provider}` – no candidate satisfied the
      `available?` predicate.
  """

  @type candidate :: Tokengate.Providers.AliasProvider.t()
