defmodule Tokengate.Routing.RouterTest do
  ...
  # ---------------------------------------------------------------------------
  # Credential daily spending cap — budget-exhausted credentials leave the pool
  # ---------------------------------------------------------------------------

  describe "credential daily spending cap" do
    test "routes normally while spend is under the cap" do
      f = full_setup()
      Providers.update_credential(f.credential, %{daily_limit_usd: "100.00"})

      assert :ok =
               Tokengate.Budgets.Manager.record_credential_spend(
                 f.credential.id,
                 Decimal.new("50.00")
               )
    ...
    test "an exhausted credential is skipped and traffic fails over to the next one" do
      team = team_fixture()
      member = team_member_fixture(team) |> Repo.preload([:team])
      ...
      model_provider_fixture(model_alias, capped_provider, %{
        provider_model: "gpt-4-capped",
        priority: 1,
        credential: capped_cred
      })

      fallback_provider = provider_fixture()
      fallback_cred = credential_fixture(fallback_provider, %{status: "active"})

      fallback_ap =
        model_provider_fixture(model_alias, fallback_provider, %{
          provider_model: "gpt-4-fallback",
          priority: 2,
          credential: fallback_cred
        })

      # Burn through the capped credential's daily budget.
      assert :ok =
               Tokengate.Budgets.Manager.record_credential_spend(
                 capped_cred.id,
                 Decimal.new("10.00")
      ...
      assert {:ok, route} = Router.route(model_alias.name, member)
      assert route.model_provider.id == fallback_ap.id
      assert route.credential.id == fallback_cred.id
      assert route.model_responded == "gpt-4-fallback"
    end

    test "returns :no_available_provider when every credential is budget-exhausted" do
      f = full_setup()
      Providers.update_credential(f.credential, %{daily_limit_usd: "5.00"})
      ...
      assert {:error, :no_available_provider} = Router.route(f.model_alias.name, f.member)
    end

    test "a credential without a limit never gets budget-excluded" do
      f = full_setup()
