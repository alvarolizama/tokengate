defmodule TokengateWeb.ProvidersLiveTest do
  ...
  test "credential form has a daily spending limit field that persists", %{conn: conn} do
    provider = create_provider()
    %{user: admin, password: password} = register_admin()
    ...
    html =
      ...
      |> form("#credential-form",
        credential: %{
          provider_id: provider.id,
          name: "Limitada",
          api_key_encrypted: "sk-limitada",
          daily_limit_usd: "25.50"
        }
      )
      |> render_submit()
    ...
    [cred] = Providers.list_credentials_for_provider(provider.id)
    assert Decimal.equal?(cred.daily_limit_usd, Decimal.new("25.50"))

    # Edit form shows the limit (rounded to cents, not full numeric scale)
    view |> element("#edit-credential-#{cred.id}") |> render_click()
    assert has_element?(view, "#credential-form input[value='25.50']")

    # Clear the limit → unlimited (nil)
    html =
      view
      |> form("#credential-form",
        credential: %{
    ...
    assert html =~ "Credencial actualizada."
    [updated] = Providers.list_credentials_for_provider(provider.id)
    assert is_nil(updated.daily_limit_usd)
  end

  test "credential form rejects a negative daily limit", %{conn: conn} do
    provider = create_provider()
    %{user: admin, password: password} = register_admin()
    ...
    html =
      ...
      |> form("#credential-form",
        credential: %{
          provider_id: provider.id,
          name: "Negativa",
          api_key_encrypted: "sk-neg",
          daily_limit_usd: "-5"
        }
      )
      |> render_submit()

    assert html =~ "debe ser mayor o igual a 0"
